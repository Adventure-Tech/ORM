...
class PersistenceManager
{
    ...
    public static function insert(object $entity): void
    {
        ...
        $isAutoIncrementId = $hasAutogeneratedId && !$entityReflection->isUlid();
        $isAutogeneratedUlid = $hasAutogeneratedId && $entityReflection->isUlid();
        if ($hasAutogeneratedId && EntityAccessorService::issetId($entity)) {
            throw new IdSetForInsertException();
        }
        if (!$hasAutogeneratedId) {
            if ($entityReflection->isUlid() && isset($entity->id) && !Str::isUlid($entity->id)) {
                throw new MissingIdValueException('Id column value is missing or is not an ULID');
            }
        ...
        } elseif ($isAutogeneratedUlid) {
            EntityAccessorService::setId($entity, Str::ulid());
            $data[$idProperty] = $entity->id;
            $query->insert($data);
        } else {
            $query->insert($data);
        }
