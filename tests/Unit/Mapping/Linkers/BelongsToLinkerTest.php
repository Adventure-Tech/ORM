...
test('Linker exposes the target entity name', function () {
    $linker = new BelongsToLinker('TargetEntity', 'relation', 'foreign_key');
    expect($linker->getTargetEntity())->toBe('TargetEntity');
});

test('Linker exposes the foreign key', function () {
    $linker = new BelongsToLinker('TargetEntity', 'relation', 'foreign_key');
    expect($linker->getForeignKey())->toBe('foreign_key');
});
...
test('Linker can apply join correctly', function (LocalAliasingManager $manager, array $filters, string $expectedQueryString, array $expectedBindings) {
    ...
    ->with([
        [
            [],
            'select * left join "table_name" as "table_alias" on "qualified"."id" = "qualified"."foreign_key"',
            [],
        ],
        [
           [new class implements Filter {
            ...
            'select * left join "table_name" as "table_alias" on "qualified"."id" = "qualified"."foreign_key" and "filter_column" = ?',
            ['filter_value'],
        ],
    ]);
